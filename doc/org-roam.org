...
It is common to take notes on academic reference meterial. To designate a node
as the canonical node for a particular reference, we can use its unique citation
key:
...
6. ~:target~ is a compulsory specification in the Org-roam capture template. The
   ...
   element indicates the location of the captured node (note the target location
   undergoes Org template expansion like the body elements), and the rest of the
