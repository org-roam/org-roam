...
=org-roam-capture-templates=. Just like the base =org-capture= this
variable can contain multiple templates, in which case you will be prompted on which
one to use when capturing a new note.
...
6. =:file-name= is the file-name template for a new note, if it doesn't yet
   ...
   allows you to specify if you want the note to go into a
   subdirectory. e.g. the template =private/${slug}= will create notes in
